...
for n in nst:
    data = loadmat('time' + n + '.mat')
    multtime = data['multtime']
    setuptime = data['setuptime']
    
    plt.figure(1)
    plt.loglog(N,setuptime,  linewidth = 2., label = r'$\kappa_{%s}$'%(n))
    
    plt.figure(2)
    plt.loglog(N,multtime,  linewidth = 2., label = r'$\kappa_{%s}$'%(n))
...
plt.legend()
plt.savefig('figs/setuptimekernelcomp.png')
#cplt.show()
...
plt.legend()
plt.savefig('figs/matvectimekernelcomp.png')
plt.show()
